    ...
    def requestEmitir(e):
        ...
        if driver.title!='Empresas':
            ...
            driver.execute_script("""
                                  ...
                                  """)
            sleep(1)
            driver.execute_script("if(document.querySelector('iframe#iframe').contentWindow.document.querySelector('body').className==''){console.log('ERROR')}else{console.log('RETURN')}")
            sleep(5)
            log=driver.get_log('browser')
            print(log)
            try:
